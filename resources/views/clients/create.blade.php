  ...
  <div class="mx-auto w-full bg-slate-900 dark:bg-gray-100">
    <div class="flex justify-between items-center mb-4 p-4 dark:bg-white bg-gray-800 w-full">
      <div class="flex flex-col gap-2 md:flex-row w-full">
        ...
        <h1 class="text-2xl font-bold ">Add a Client</h1>
      </div>
    ...
    <div class="max-w-2xl mx-auto p-6 dark:bg-white bg-slate-800 shadow-md rounded-lg">
      ...
      <form action="{{ route('clients.store') }}" method="POST">
        @csrf
        <div class="my-4">
          <label for="first_name" class="block dark:text-gray-700 text-gray-100 mb-1 text-lg ">First Name :</label>
        ...
        <div class="my-4">
          <label for="last_name" class="block dark:text-gray-700 text-gray-100 mb-1 text-lg ">Last Name :</label>
          <input type="text" id="last_name" name="last_name" value="{{ old('last_name') }}"
            class="w-full mb-2 px-2 py-2 rounded focus:outline-none dark:text-black text-white bg-slate-700 dark:bg-gray-200 ">
        </div>
        <div class="grid gap-2 grid-cols-1 lg:grid-cols-2">
          <div class="mb-4">
            <label for="phone" class="block dark:text-gray-700 text-gray-100 mb-1 text-lg ">Phone :</label>
            <input type="tel" id="phone" name="phone" value="{{ old('phone') }}"
              class="w-full mb-2 px-2 py-2 rounded focus:outline-none dark:text-black text-white bg-slate-700 dark:bg-gray-200 ">
          </div>
          <div class="mb-4">
            <label for="city" class="block dark:text-gray-700 text-gray-100 mb-1 text-lg">City :</label>
            <select name="city" id="city"
              class="w-full mb-2 p-2 rounded focus:outline-none dark:text-black text-white bg-slate-700 dark:bg-gray-200 ">
              <option value="">Select a city</option>
              @foreach ($cities as $item)
                <option value="{{ $item }}" {{ old('city') == $item ? 'selected' : '' }}>{{ $item }}
                </option>
              @endforeach
            </select>
          </div>
        ...
        <div class="mb-4">
          <label for="birthday" class="block dark:text-gray-700 text-gray-100 mb-1 text-lg">Birthday
            :</label>
