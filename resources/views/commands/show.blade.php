  ...
  <div class="mx-auto w-full bg-slate-900 dark:bg-gray-100">
    <div class="flex justify-between items-center mb-4 p-4 dark:bg-white bg-slate-800 w-full">
      <div class="flex flex-col gap-2 md:flex-row w-full">
        ...
        <h1 class="text-2xl font-bold ">Command with id : {{ $command->id }}</h1>
      </div>
    ...
    <div class="flex flex-col md:flex-row gap-2 justify-center md:items-start items-stretch">
      <div class="max-w-2xl p-6 dark:bg-white bg-slate-800 shadow-md rounded-lg mt-10">
        <h1 class="text-2xl font-bold dark:text-slate-800 text-slate-200 mb-4">Details of command with id :
          {{ $command->id }}</h1>
        <table class="min-w-full dark:bg-white bg-slate-800 dark:text-gray-700 text-gray-300">
          ...
          <tbody>
            <tr class="w-full border-b dark:border-gray-700">
              ...
              <td class="py-2 px-4 ">{{ $command->client->first_name }}</td>
            </tr>
            <tr class="w-full border-b dark:border-gray-700">
              <td class="py-2 px-4 font-semibold ">Date :</td>
              <td class="py-2 px-4 ">{{ $command->date }}</td>
            </tr>
            <tr class="w-full border-b dark:border-gray-700">
              <td class="py-2 px-4 font-semibold ">Amount :</td>
              <td class="py-2 px-4 ">{{ $command->amount }} DH</td>
            </tr>
          </tbody>
        </table>
        ...
        <div class="w-full flex justify-end flex-wrap gap-2 py-4 font-bold">
          ...
          <button
            ...
            data-id="{{ $command->id }}" title="Delete">Delete</button>
        </div>
      </div>
      @if ($command->products && $command->products->count() > 0)

        <div class="max-w-2xl p-6 dark:bg-white bg-slate-800 shadow-md rounded-lg mt-10">
          <h1 class="text-2xl font-bold dark:text-slate-800 text-slate-200 mb-4">Purchases:</h1>
          <table class="min-w-full dark:bg-white bg-slate-800 dark:text-gray-700 text-gray-300">
            <thead class="bg-slate-700 text-white dark:bg-gray-200 dark:text-black rounded-md">
              <tr>
                <th class="py-2 px-4 font-semibold text-start">Product</th>
                <th class="py-2 px-4 font-semibold dark:text-gray-700 :text-gray-300 text-start">Price</th>
                <th class="py-2 px-4 font-semibold dark:text-gray-700 :text-gray-300 text-start">Quantity</th>
              </tr>
            </thead>
            <tbody>
              @foreach ($command->products as $item)
                <tr class="w-full border-b dark:border-gray-700">
                  ...
                  <td class="py-2 px-4 ">{{ $item->pivot->quantity }}</td>
                </tr>
              @endforeach
